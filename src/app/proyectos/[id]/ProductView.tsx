  ...
  return (
    <section className="min-h-screen">
      <div className="px-4 sm:px-10 py-10 sm:py-20">
        <h1 className="text-5xl sm:text-6xl smd:text-7xl lg:text-8xl font-zodiak-light break-words hyphens-auto uppercase text-brown-100">
          {title}
        </h1>
        <div className="flex flex-row flex-wrap w-full mt-6 gap-4">
          {tags.map((tag) => (
            <span
              className="p-2 bg-brick text-primary font-zodiak-regular rounded-full"
              key={tag}
            >
              {tag}
            </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 mt-4 gap-4 px-12 pb-12">
        {photo_urls.map((url) => (
          <img src={url} className="col-span-1" key={url} />
        ))}
      </div>
      <div className="mt-4 w-full sm:w-2/3 px-8 sm:px-12 pb-12">
